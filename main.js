...
const setStatusClosed = id => {
  ...
  document.getElementById("classChange").className = "label label-warning";
  localStorage.setItem('issues', JSON.stringify(issues));
  fetchIssues();
}
...
const fetchIssues = () => {
  ...
  for (var i = 0; i < issues.length; i++) {
    ...
    const issueClosed = document.querySelector("#issueClosed");
    const issueOpen = document.querySelector("#issueOpen");

    totalIssues.innerHTML = issues.length;
    issueOpen.innerHTML = issues.length;
}
